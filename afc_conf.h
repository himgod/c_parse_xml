...
struct afi_policy_conf{
	...
	BOOL net_adaption;
	BOOL access_control;
	struct block_list_conf afi_list;
};

struct user_policy_conf{
	char auto_optim_policy[MID_STRING_LEN];	
	struct block_list_conf user_list;
};
struct wlan_conf{
...
void destroy_user_black_list(struct block_list_conf * block_hash);
void destroy_afi_black_list(struct block_list_conf * block_hash);
